...
func NewUsersUsecase(txRepo TxRepo, repo UsersRepo, accountRepo AccountRepo) *UsersUsecase {
	return &UsersUsecase{txRepo: txRepo, repo: repo, accountRepo: accountRepo}
}
...
func (uc *UsersUsecase) FindOrCreateUser(ctx context.Context, dto *UserDTO) (*UserProfileVO, bool, error) {
	...
	if err != nil {
		log.Error("Get account by email and provider failed", slog.Any("error", err))
	...
	if existAcc != nil {
		...
		if err != nil {
			log.Error("Get user by uid failed", slog.Any("error", err))
			return nil, false, err
		}
		if existUser == nil {
	...
	err = uc.txRepo.ExecFunc(func(tx *gorm.DB) error {
		...
		for range maxCount {
			...
			if err != nil {
				...
				if errors.As(err, &mysqlErr) && mysqlErr.Number == 1062 {
					log.Info("UserUID already exists, retrying", slog.Any("userUID", user.UID))
					continue
				}
				log.Error("Create user failed", slog.Any("error", err))
				return err
			}
			break
		}
		...
		if err != nil {
			log.Error("Create account failed", slog.Any("error", err))
			return err
		}
	...
	if err != nil {
		log.Error("Create user and account failed", slog.Any("error", err))
		return nil, false, err
	}

...
func NewUserSessionService(sessionConf *conf.Session, userSessionRepo biz.UserSessionRepo, cacheService *CacheService, jwtService *JwtService) *UserSessionService {
...
func (s *UserSessionService) CreateUserSession(ctx context.Context, userSession *model.UserSession) error {
	if err := s.userSessionRepo.CreateUserSession(ctx, userSession); err != nil {
		log.Error("Create user session failed", "error", err)
		return err
	}
	return nil
}
...
func (s *UserSessionService) DeleteUserSession(ctx context.Context, sessionId string) error {
	...
	if err != nil {
		log.Error("Find user session by session id failed", "error", err)
		return err
	}
	// 2. 如果会话存在, 则删除会话
	if session != nil {
		if err := s.userSessionRepo.DeleteUserSessionBySessionID(ctx, sessionId); err != nil {
			log.Error("Delete user session by session id failed", "error", err)
			return err
		}
	}
	// 3. 删缓存
...
func (s *UserSessionService) ValidateSession(ctx context.Context, sessionId string) error {
	...
	if err != nil {
		log.Error("Get user session info failed", "error", err)
		return err
	}
	// map不为空, 则会话有效
	...
	if err := s.cacheService.SetUserSession(ctx, session.UID, sessionId, expiration); err != nil {
		log.Error("Set user session failed", "error", err)
		return err
	}
	return nil
}
...
func (s *UserSessionService) LimitUserSession(ctx context.Context, UID int) error {
	...
	if err != nil {
		log.Error("Find user session by user id failed", "error", err)
		return err
	}
	// 4. 会话数量超过限制, 删除最早的会话
	if len(sessions) > int(limitCount) {
		...
		if err := s.userSessionRepo.DeleteUserSessionByIDs(ctx, deleteSessionIds); err != nil {
			log.Error("Delete user session by ids failed", "error", err)
			return err
		}
		// 删除对应的缓存
